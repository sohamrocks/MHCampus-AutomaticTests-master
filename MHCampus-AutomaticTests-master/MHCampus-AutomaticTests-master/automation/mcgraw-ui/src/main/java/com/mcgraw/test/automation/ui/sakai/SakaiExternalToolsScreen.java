...
@PageFrameIdentificator(locators = @DefinedLocators({
		@DefinedLocator(how = How.CSS, using = ".portletMainWrap iframe") }))
...
public class SakaiExternalToolsScreen extends Screen {
	...
	//added tool to activate checkbox for Gb connection
	@DefinedLocators(@DefinedLocator(how = How.ID, using = "sakai.gradebook.tool"))
	Element gradebookToolBtn;
	...
	@DefinedLocators(@DefinedLocator(how = How.XPATH, using = "//*/span[contains(text(),'MH Campus')]"))
	...
	public SakaiSetupExternalToolsScreen clickSetupBtn() {
		...
		//checks if checkbox is turned off and activates it
		if (!gradebookToolBtn.isSelected()) {
			gradebookToolBtn.waitForPresence(5);
			gradebookToolBtn.click();
		}
		...
		mhCampusLink.waitForPresence(5);
		mhCampusLink.click();
		browser.makeScreenshot();
