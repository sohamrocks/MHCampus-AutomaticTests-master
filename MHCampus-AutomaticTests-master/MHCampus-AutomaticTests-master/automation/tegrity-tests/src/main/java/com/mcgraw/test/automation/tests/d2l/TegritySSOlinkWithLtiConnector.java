...
public class TegritySSOlinkWithLtiConnector extends BaseTest {
	...
	protected static String moduleName = "Module" + RandomStringUtils.randomNumeric(6);
	...
	public void testSuiteSetup() throws Exception {
	    ...
	    tegrityInstanceConnectorsScreen.configureCustomAuthorizationConnector(d2lApplication.d2lLtiTitle, d2lApplication.d2lLtiServiceUrl, 
	    		         d2lApplication.d2lLtiExtendedProperties.replace("<customer_number>", tegrityInstanceApplicationNoLocalConnector.customerNumber));
		...
		d2lApplication.createD2lLinkConnectedWithInstance(linkName, courseId1, tegrityInstanceApplicationNoLocalConnector.customerNumber,
				                  tegrityInstanceApplicationNoLocalConnector.sharedSecret);		
		d2lApplication.addCreatedLinkToD2lModule(courseId1, linkName, moduleName);	
		
		d2lApplication.createD2lLinkConnectedWithInstance(linkName, courseId2, tegrityInstanceApplicationNoLocalConnector.customerNumber,
                tegrityInstanceApplicationNoLocalConnector.sharedSecret);		
		d2lApplication.addCreatedLinkToD2lModule(courseId2, linkName, moduleName);	
	}
	...
	public void checkTegrityLinkIsPresentInInstructorsCourse() throws Exception {
		...
		Assert.assertEquals(d2lContentCourseScreen.getTegrityLinksCount(), 1, "Wrong count of Tegrity links for instructor's course "
				+ courseName1);
	}
	...
	public void checkTegrityLinkIsPresentInStudentsCourse() throws Exception {
		...
		d2lContentCourseScreen = d2lCourseDetailsScreen.clickContentLink();
		d2lContentCourseScreen.chooseModuleBlock(moduleName);
		Assert.assertEquals(d2lContentCourseScreen.getTegrityLinksCount(), 1, "Wrong count of Tegrity links for student's course "
				+ courseName1);
	}
	...
	public void checkTegrityLinkBehavesCorrectlyForD2LInstructor() throws Exception {
		...
		d2lContentCourseScreen = d2lCourseDetailsScreen.clickContentLink();
		d2lContentCourseScreen.chooseModuleBlock(moduleName);
		tegrityCourseDetailsScreen = d2lContentCourseScreen.clickTegrityLink();
