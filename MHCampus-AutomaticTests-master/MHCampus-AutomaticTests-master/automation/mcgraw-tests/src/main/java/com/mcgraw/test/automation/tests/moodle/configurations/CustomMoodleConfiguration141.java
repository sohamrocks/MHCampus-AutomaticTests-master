 ...
 * Fallback to user_id and context_id: 
 * Off Asvnc: Off
 */
public class CustomMoodleConfiguration141 extends BaseTest {
	...
	private String newAssignmentId = getRandomNumber();
	...
	private String customerNumber = "TBNB-F17K-SO0R";
	private String institution = "CustomMoodleConfiguration141";
	private String sharedSecret = "CDBDEB";
	private String instPassword = "notrahke";

	@BeforeClass
	public void testSuiteSetup() throws Exception {
		Logger.info("Starting test for configuration: CustomMoodleConfiguration141");
	...
	@Test(description = "Check TestScorableItem form is submitted successfully", dependsOnMethods = "checkSsoAsStudentMoodle")
	...
	@Test(description = "Check TestScorableItem form is submitted successfully with update assignment ID", dependsOnMethods = {
			"checkSubmittingTestScorableItemFormIsSuccessfull" })
	public void checkSubmittingTestScorableItemFormIsSuccessfullUpdateAssignmentId() throws Exception {
		// fill the form with new assignment ID

		TestScoreItemsScreen testScoreItemsScreen = gradebookApplication.fillTestScorableItemForm(customerNumber,
				providerId, courseRS1.getId(), newAssignmentId, assignmentTitle, categoryType, description, startDate,
				dueToDate, scoreType, scorePossible, isStudentViewable, isIncludedInGrade,
				gradebookApplication.tegrityServiceLocation);
		testScoreItemsScreen.submitForm();

		Assert.assertTrue(testScoreItemsScreen.formSubmitIsSuccessfullForMoodle(), "TestScoreItems form submit failed");
		// Check that Assignment title does not change
		moodleHomeScreen = moodleApplication.loginToMoodle(studentLogin1, password);
		moodleCourseScreen = moodleHomeScreen.clickOnAllCoursesLink();
		moodleCourseDetailsScreen = moodleCourseScreen.findAndSelectCourse(courseFullName1);
		moodleGradesScreen = moodleCourseDetailsScreen.clickGradesLink();
		Assert.verifyEquals(
				browser.findElement(By.xpath("//*[@class='level3 levelodd item b1b column-itemname']/span")).getText(),
				assignmentTitle, "Assignment title does not match");
	}

	@Test(description = "Check TestScore form is submitted successfully", dependsOnMethods = {
			"checkSubmittingTestScorableItemFormIsSuccessfullUpdateAssignmentId" })
	public void checkSubmittingTestScoreFormIsSuccessfullForStudent1() throws Exception {
		...
		TestScoreScreen testScoreScreen = gradebookApplication.fillTestScoreForm(customerNumber, providerId,
				courseRS1.getId(), newAssignmentId, studentLogin1, commentForStudent1, dateSubmitted,
