...
* Async:  Off
*/
public class CustomCanvasConfiguration83 extends BaseTest {
	@Autowired
	...
	private String customerNumber = "6C6Z-F3TF-THFU";
	private String sharedSecret = "C21AC5";
	...
	public void testSuiteSetup() throws Exception {
		...
		Logger.info("Course ID – User ID Mapping:  internal – SIS  | Gradebook Connector: Yes | SSO Connector: No | Canvas Mapping: Yes ");
		...
		//classic
		 ...
		 canvasCourseDetailsScreen.createMhCampusApplication(appName,customerNumber, sharedSecret,canvasApplication.ltiLaunchUrl , canvasMappingType.INTERNALTOSIS , true);
	...
	public void checkSubmittingTestScoreFormIsSuccessfullForStudent() throws Exception {
		...
		TestScoreScreen testScoreScreen = gradebookApplication.fillTestScoreForm(customerNumber, providerId,
				Integer.toString(course1.getId()), assignmentId, stuSisId, commentForStudent,
	...
	private void prepareTestDataInCanvas() throws Exception {
		...
		studentWithSisEnrollment = canvasApiUtils.enrollToCourseAsActiveStudent(studentWithSis, course1);
		instructorWithSisEnrollment = canvasApiUtils.enrollToCourseAsActiveTeacher(instructorWithSis, course1);	
		
		studentWithSisEnrollment2 = canvasApiUtils.enrollToCourseAsActiveStudent(studentWithSis2, course2);
		instructorWithSisEnrollment2 = canvasApiUtils.enrollToCourseAsActiveTeacher(instructorWithSis2, course2);

	}

	private void clearCanvasData() throws Exception {

		if ((studentWithSisEnrollment != null) & (course1 != null))
			canvasApiUtils.deleteEnrollment(studentWithSisEnrollment, course1);
		if ((studentWithSisEnrollment2 != null) & (course2 != null))
			canvasApiUtils.deleteEnrollment(studentWithSisEnrollment2, course2);

		if ((instructorWithSisEnrollment != null) & (course1 != null))
			canvasApiUtils.deleteEnrollment(instructorWithSisEnrollment, course1);
		if ((instructorWithSisEnrollment2 != null) & (course2 != null))
			canvasApiUtils.deleteEnrollment(instructorWithSisEnrollment2, course2);
