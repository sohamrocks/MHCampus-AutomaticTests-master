...
public class BlackboardApplication {
	...
	public void completeTegritySetupWithBlackBoard(String customerNumber,
			String sharedSecret) {
	...
	public BlackboardHomeScreen loginToBlackBoard(String userName,
		...
		Logger.info("After deletion cookies are empty = " + afterDeletion.isEmpty());

		try{
			blackboardLoginScreen = browser.openScreen(blackboardUrl, BlackboardLoginScreen.class);
		}catch(Exception e){
			logoutFromBlackBoard();
			blackboardLoginScreen = browser.openScreen(blackboardUrl, BlackboardLoginScreen.class);
		}
